...
(defn find-carts
  ...
  (->> (for [[y line] (map-indexed vector graph)
         ...
         {:x x, :y y, :sym point, :n-turns 0})
       (map-indexed (fn [idx m] [idx (assoc m :id idx)]))
       (into {})))
...
(defn transform-cart
  [graph {:keys [x y n-turns sym], :as cart}]
  (let [[new-y new-x] (case sym
                        \^ [(dec y) x]
                        \v [(inc y) x]
                        \< [y (dec x)]
                        \> [y (inc x)])
                                  ...
                                  (some
                                   (fn [[id m]]
                                     (when (and (= (:x new-cart) (:x m))
                                       ...
                                       id))))]
                         [(cond-> acc-collisions
                            collision-id (conj cart-id collision-id))
                          (assoc acc-carts cart-id new-cart)]))
                     [#{} carts]))]
    (if remove?
      (->> new-carts
           (remove (fn [[id _]] (contains? collisions id)))
           (into {}))
      (->> new-carts
           (map
            (fn [[id cart]] [id
...
(defn print-graph
  ...
  (let [xy->cart (-> carts
    ...
    (doseq [[y line] (map-indexed vector graph)]
      (println (apply str
                      (for [[x point] (map-indexed vector line)]
...
(defn solution-2
  ...
  (let [graph (read-input)
        ...
        final-generation (some (fn [generation]
                                 (when (= 1 (count generation)) generation))
                               generations)]
    (str (-> final-generation
             first
             second
             :x)
         ","
